...
class MyTrainDataset(Dataset):
    ...
    def extract_background_color(self, image, padding=20, threshold=50):
        top = image[:padding, :, :]
        ...
        if black_ratio >= 0.8:
            ...
            background_color = np.median(non_black_edges, axis=0).astype(np.uint8)

        return background_color, edges


    def perlin_synthetic(self, image, background_color, thresh, anomaly_source_path, cv2_image,thresh_path):

        no_anomaly = torch.rand(1).numpy()[0]
        if no_anomaly > 0.6:
            image = image.astype(np.float32)
            perlin_noise = np.zeros((self.resize_shape[0], self.resize_shape[1]), dtype=np.float32)
            perlin_thr = np.zeros((self.resize_shape[0], self.resize_shape[1]), dtype=np.float32)

            return image, np.zeros((self.resize_shape[0], self.resize_shape[1], 1), dtype=np.float32), np.array([0.0], dtype=np.float32), perlin_noise, perlin_thr
            ...
            img_object_thr = np.ones_like(image) * background_color
            img_object_thr = img_object_thr.astype(np.float32) * object_perlin / 255.0

            beta = torch.rand(1).numpy()[0] * 0.6 + 0.2
            augmented_image = image * (1 - object_perlin) + img_object_thr


            return augmented_image, msk, np.array([has_anomaly], dtype=np.float32), perlin_noise, perlin_thr
    ...
    def __getitem__(self, idx):
        ...
        # 15%의 패딩값 지정
        padding = int(min(self.resize_shape[1], self.resize_shape[0]) * 0.15)
        background_color, edges = self.extract_background_color(image, padding)
        ...
        augmented_image, anomaly_mask, has_anomaly, perlin_noise, perlin_thr = self.perlin_synthetic(image,background_color,thresh,anomaly_path,cv2_image,thresh_path)
